...
function websiteIntakeText(c,project,assets=[]){
  ...
  out.push("## Website Assets / Files",assets.length?assets.map(x=>`- ${x.name}`).join("\n"):"No website assets uploaded yet.","");
  return out.join("\n");
}
function websiteProjectStatusLabel(status){return({draft:"Draft",in_progress:"In Progress",ready:"Ready for Build",complete:"Complete"})[status]||"Draft"}
...
function websiteProjectAdminCard(c,project,assets){return `<article class="website-project-card glass"><div class="website-project-card-head"><div><span class="section-label">WEBSITE PROJECT</span><h4>${esc(project.website_name||"Untitled Website")}</h4><p class="muted">Updated ${fmtDate(project.updated_at)}</p></div><div class="row-actions">${websiteProjectStatusPill(project.status)}${project.showcase_published?'<span class="status complete">Showcase Live</span>':''}<button class="mini-btn" data-action="manage-project-showcase" data-id="${project.id}" data-client-id="${c.id}">Manage Showcase</button><button class="mini-btn" data-action="copy-project-intake" data-id="${project.id}" data-client-id="${c.id}">Copy Full Intake</button></div></div><details class="admin-project-details"><summary>View complete intake</summary><div class="intake-admin-preview">${websiteIntakeAdminHtml(project)}</div><div class="project-assets-admin"><div class="section-head"><div><span class="section-label">PROJECT ASSETS</span><h4>${assets.length} file${assets.length===1?"":"s"}</h4></div></div>${websiteProjectAssetAdminListHtml(assets,c,project)}</div></details></article>`}
...
async function renderEmployerWebsiteProjectEditor(project,editable){
  const c=currentClient(),seq=state.navSeq;const [assets,showcase]=await Promise.all([listWebsiteProjectAssets(c.id,project.id),buildShowcaseView(project)]);if(seq!==state.navSeq)return;const intake=project.website_intake&&typeof project.website_intake==="object"?project.website_intake:{};
  $("view").innerHTML=`${showcase}<div class="project-editor-top"><button class="btn ghost" data-action="back-to-website-projects">← All Website Projects</button>${editable?`<button class="mini-btn danger" data-action="delete-website-project" data-id="${project.id}">Delete Project</button>`:""}</div><article class="card glass website-project-hero"><div><span class="section-label">WEBSITE PROJECT</span><h3>${esc(project.website_name||"Untitled Website")}</h3><p class="muted">Complete the intake below. Your answers are saved only to this website project.</p></div>${websiteProjectStatusPill(project.status)}</article><article class="card glass" style="margin-top:13px"><div class="form-grid"><div class="field"><span>Name of Website</span><input id="websiteProjectName" value="${esc(project.website_name||"")}" placeholder="Business / website name" ${editable?"":"disabled"}></div><div class="field"><span>Project Status</span><select id="websiteProjectStatus" ${editable?"":"disabled"}><option value="draft" ${project.status==="draft"?"selected":""}>Draft</option><option value="in_progress" ${project.status==="in_progress"?"selected":""}>In Progress</option><option value="ready" ${project.status==="ready"?"selected":""}>Ready for Build</option><option value="complete" ${project.status==="complete"?"selected":""}>Complete</option></select></div></div></article><div class="intake-accordion">${WEBSITE_INTAKE_SECTIONS.map((section,i)=>`<details class="intake-section glass" ${i===0?"open":""}><summary><div><span class="intake-number">${String(i+1).padStart(2,"0")}</span><strong>${esc(section.title.replace(/^\d+\.\s*/,""))}</strong></div><span class="intake-chevron">⌄</span></summary><div class="intake-body"><p>${esc(section.help)}</p><ul>${section.items.map(x=>`<li>${esc(x)}</li>`).join("")}</ul><textarea class="auto-grow intake-answer" data-intake-id="${section.id}" rows="5" placeholder="Type or paste your answer here…" ${editable?"":"disabled"}>${esc(intakeValue(intake,section.id))}</textarea></div></details>`).join("")}</div><details class="intake-section glass intake-send-guide" style="margin-top:13px"><summary><div><span class="intake-number">✓</span><strong>What to Send Us</strong></div><span class="intake-chevron">⌄</span></summary><div class="intake-body"><p>You can send everything in whichever format is easiest. You do not need to organize everything perfectly.</p><ul><li>Google Drive folder</li><li>Dropbox</li><li>Photos</li><li>Screenshots</li><li>PDFs</li><li>Google Docs</li><li>Existing website links</li><li>Social media links</li><li>Booking links</li><li>Menus</li><li>Price lists</li><li>Notes</li></ul></div></details><article class="card glass section-panel" style="margin-top:13px"><span class="section-label">MORE INFORMATION</span><h3>Additional Notes</h3><textarea id="websiteProjectNotes" class="auto-grow soft-textarea" rows="6" placeholder="Anything else we should know about this website…" ${editable?"":"disabled"}>${esc(project.website_notes||"")}</textarea>${editable?'<div class="card-actions start"><button class="btn primary" data-action="save-website-project" data-id="'+project.id+'">Save Project Intake</button></div>':""}</article><article class="card glass" style="margin-top:13px"><div class="section-head"><div><span class="section-label">PROJECT ASSETS</span><h3>Logos, photos & source files</h3><p class="muted">These files belong only to <strong>${esc(project.website_name||"this project")}</strong>.</p></div></div>${editable?`<label class="website-dropzone" id="websiteAssetDrop"><strong>＋ Upload Project Assets</strong><span>Photos, logos, SVG, PDF, DOC/DOCX · up to 25 MB each</span><input id="websiteAssetInput" type="file" multiple accept="image/*,.svg,.pdf,.doc,.docx" hidden></label>`:""}<div class="file-grid website-assets">${websiteProjectAssetListHtml(assets,c,project,editable)}</div></article>`;
  setupAutoGrow($("view"));setupWebsiteProjectDropzone(project.id);startShowcaseCarousels();
}
async function saveWebsiteProject(projectId){const c=currentClient();if(!c)return;const website_intake={};$$('[data-intake-id]').forEach(el=>website_intake[el.dataset.intakeId]=el.value.trim());const payload={website_name:$("websiteProjectName")?.value.trim()||"Untitled Website",website_intake,website_notes:$("websiteProjectNotes")?.value.trim()||"",status:$("websiteProjectStatus")?.value||"draft",updated_at:new Date().toISOString()};const {error}=await state.sb.from("website_projects").update(payload).eq("id",projectId).eq("client_id",c.id);if(error)return toast(error.message);toast("Website project saved");state.activeWebsiteProject=projectId;renderEmployer("website")}
async function deleteWebsiteProject(projectId){const c=currentClient();if(!c||!confirm("Delete this website project and its project record? Uploaded project assets will also be removed first."))return;const assets=await listWebsiteProjectAssets(c.id,projectId);if(assets.length){const paths=assets.map(f=>`${c.id}/website-projects/${projectId}/${f.name}`);const {error:storageError}=await state.sb.storage.from("client-files").remove(paths);if(storageError)return toast(storageError.message)}const {error}=await state.sb.from("website_projects").delete().eq("id",projectId).eq("client_id",c.id);if(error)return toast(error.message);state.activeWebsiteProject=null;toast("Website project deleted");renderEmployer("website")}
async function listWebsiteProjectAssets(clientId,projectId){if(!clientId||!projectId)return[];const {data,error}=await state.sb.storage.from("client-files").list(`${clientId}/website-projects/${projectId}`,{limit:100,sortBy:{column:"created_at",order:"desc"}});if(error){console.warn(error);return[]}return(data||[]).filter(x=>x.name!==".emptyFolderPlaceholder")}
...
function websiteProjectAssetListHtml(files,c,project,editable){return files.length?files.map(f=>{const path=`${c.id}/website-projects/${project.id}/${f.name}`;return`<div class="file-card"><div><strong>${esc(f.name)}</strong><small class="muted">${humanSize(f.metadata?.size||0)}</small></div><div class="row-actions"><button class="mini-btn" data-action="open-file" data-path="${esc(path)}">Open</button>${editable?`<button class="mini-btn danger" data-action="delete-project-asset" data-path="${esc(path)}" data-project-id="${project.id}">Delete</button>`:""}</div></div>`}).join(""):'<div class="empty website-assets-empty"><strong>No project assets uploaded yet</strong>Logos, photos, screenshots and documents will appear here.</div>'}
function websiteProjectAssetAdminListHtml(files,c,project){return `<div class="file-grid website-assets">${files.length?files.map(f=>{const path=`${c.id}/website-projects/${project.id}/${f.name}`;return`<div class="file-card"><div><strong>${esc(f.name)}</strong><small class="muted">${humanSize(f.metadata?.size||0)}</small></div><div class="row-actions"><button class="mini-btn" data-action="open-file" data-path="${esc(path)}">Open</button></div></div>`}).join(""):'<div class="empty website-assets-empty"><strong>No assets for this project</strong></div>'}</div>`}
...
async function uploadWebsiteProjectAssets(files,projectId){const c=currentClient();if(!c||!files?.length||!projectId)return;const maxBytes=25*1024*1024;for(const f of files){if(f.size>maxBytes)return toast(`${f.name} exceeds the 25 MB limit`);const safe=f.name.replace(/[^\w.\- ]+/g,"_");const name=`${Date.now()}-${crypto.randomUUID().slice(0,8)}-${safe}`;const {error}=await state.sb.storage.from("client-files").upload(`${c.id}/website-projects/${projectId}/${name}`,f,{upsert:false});if(error)return toast(error.message)}toast("Project assets uploaded");state.activeWebsiteProject=projectId;renderEmployer("website")}
async function deleteWebsiteProjectAsset(path,projectId){if(!confirm("Delete this project asset?"))return;const {error}=await state.sb.storage.from("client-files").remove([path]);if(error)return toast(error.message);toast("Asset deleted");state.activeWebsiteProject=projectId;renderEmployer("website")}
function setupWebsiteProjectDropzone(projectId){const zone=$("websiteAssetDrop"),input=$("websiteAssetInput");if(!zone||!input)return;["dragenter","dragover"].forEach(ev=>zone.addEventListener(ev,e=>{e.preventDefault();zone.classList.add("dragging")}));["dragleave","drop"].forEach(ev=>zone.addEventListener(ev,e=>{e.preventDefault();zone.classList.remove("dragging")}));zone.addEventListener("drop",e=>uploadWebsiteProjectAssets([...e.dataTransfer.files],projectId));input.addEventListener("change",()=>uploadWebsiteProjectAssets([...input.files],projectId))}
...
async function buildShowcaseView(project){
  ...
  const gallery=showcaseGalleryPaths(project),paths=[project.showcase_logo_path,...gallery].filter(Boolean);
  const urls=await Promise.all(paths.map(p=>signedFileUrl(p)));
  let cursor=0,logoUrl=project.showcase_logo_path?urls[cursor++]:"";const galleryUrls=gallery.map(()=>urls[cursor++]).filter(Boolean);
  const site=project.showcase_url?websiteHref(project.showcase_url):"#";
  return `<section class="project-showcase glass"><div class="showcase-top"><div><span class="section-label">ONGOING WEBSITE PREVIEW</span><h2>${esc(project.website_name||"Website Project")}</h2><p class="muted">${esc(project.showcase_notes||"Preview the latest design direction and website progress below.")}</p>${site!=="#"?`<a class="btn primary showcase-site-link" href="${esc(site)}" target="_blank" rel="noopener">Open Website ↗</a>`:""}</div>${logoUrl?`<button class="showcase-logo-card" data-action="open-showcase-image" data-url="${esc(logoUrl)}" data-mode="logo" aria-label="Enlarge website logo"><span>Website Logo</span><img src="${esc(logoUrl)}" alt="${esc(project.website_name||"Website")} logo"></button>`:""}</div>${galleryUrls.length?`<div class="showcase-gallery-head"><div><span class="section-label">LATEST SCREENSHOTS</span><h3>Website Gallery</h3></div><span class="muted">Tap any image to enlarge</span></div><div class="showcase-carousel" data-showcase-carousel><div class="showcase-track">${galleryUrls.map((url,i)=>`<button class="showcase-slide" data-action="open-showcase-image" data-url="${esc(url)}" data-mode="screenshot" aria-label="Open screenshot ${i+1}"><img src="${esc(url)}" alt="Website screenshot ${i+1}" loading="lazy"><span>${String(i+1).padStart(2,"0")}</span></button>`).join("")}</div><div class="showcase-dots">${galleryUrls.map((_,i)=>`<i class="${i===0?"active":""}"></i>`).join("")}</div></div>`:""}</section>`;
}
function startShowcaseCarousels(){
  $$('[data-showcase-carousel]').forEach(car=>{if(car._timer)clearInterval(car._timer);const track=car.querySelector('.showcase-track'),slides=[...car.querySelectorAll('.showcase-slide')],dots=[...car.querySelectorAll('.showcase-dots i')];if(slides.length<2)return;let i=0;const go=n=>{i=(n+slides.length)%slides.length;track.style.transform=`translateX(-${i*100}%)`;dots.forEach((d,x)=>d.classList.toggle('active',x===i))};car._timer=setInterval(()=>go(i+1),4200);car.addEventListener('pointerdown',()=>clearInterval(car._timer),{once:true})})
}
async function openProjectShowcaseEditor(clientId,projectId){
  const project=await getWebsiteProject(projectId);if(!project)return toast("Project not found");
  $("showcaseClientId").value=clientId;$("showcaseProjectId").value=projectId;$("showcaseUrl").value=project.showcase_url||"";$("showcaseNotes").value=project.showcase_notes||"";$("showcasePublished").checked=!!project.showcase_published;$("showcaseLogoInput").value="";$("showcaseGalleryInput").value="";
  const logoUrl=await signedFileUrl(project.showcase_logo_path);$("showcaseLogoPreview").innerHTML=logoUrl?`<div class="showcase-admin-logo"><img src="${esc(logoUrl)}" alt="Current logo"><button type="button" class="mini-btn danger" data-action="remove-showcase-logo" data-id="${projectId}">Remove Logo</button></div>`:'<div class="empty"><strong>No showcase logo yet</strong></div>';
  await renderShowcaseAdminGallery(project);openModal("showcaseModal");
}
async function renderShowcaseAdminGallery(project){
...
async function saveProjectShowcase(){
  ...
  try{let logoPath=project.showcase_logo_path||"",gallery=showcaseGalleryPaths(project);const logoFile=$("showcaseLogoInput").files?.[0];if(logoFile){const next=await uploadShowcaseFile(logoFile,clientId,projectId,"logo");if(logoPath)await state.sb.storage.from("client-files").remove([logoPath]);logoPath=next}const shots=[...($("showcaseGalleryInput").files||[])];for(const file of shots)gallery.push(await uploadShowcaseFile(file,clientId,projectId,"gallery"));const payload={showcase_url:$("showcaseUrl").value.trim(),showcase_notes:$("showcaseNotes").value.trim(),showcase_logo_path:logoPath,showcase_gallery:gallery,showcase_published:$("showcasePublished").checked,showcase_updated_at:new Date().toISOString(),updated_at:new Date().toISOString()};const {error}=await state.sb.from("website_projects").update(payload).eq("id",projectId).eq("client_id",clientId);if(error)throw error;closeModal("showcaseModal");toast("Project showcase saved");await renderClientDetail(clientId)}catch(e){toast(e.message||"Could not save showcase")}
}
async function removeShowcaseLogo(projectId){const p=await getWebsiteProject(projectId);if(!p?.showcase_logo_path)return;const {error:se}=await state.sb.storage.from("client-files").remove([p.showcase_logo_path]);if(se)return toast(se.message);const {error}=await state.sb.from("website_projects").update({showcase_logo_path:"",showcase_updated_at:new Date().toISOString()}).eq("id",projectId);if(error)return toast(error.message);await openProjectShowcaseEditor(p.client_id,projectId)}
async function removeShowcaseImage(projectId,index){const p=await getWebsiteProject(projectId);if(!p)return;const gallery=showcaseGalleryPaths(p),path=gallery[index];if(!path)return;const {error:se}=await state.sb.storage.from("client-files").remove([path]);if(se)return toast(se.message);gallery.splice(index,1);const {error}=await state.sb.from("website_projects").update({showcase_gallery:gallery,showcase_updated_at:new Date().toISOString()}).eq("id",projectId);if(error)return toast(error.message);await openProjectShowcaseEditor(p.client_id,projectId)}
...
document.addEventListener("click",async e=>{
  ...
  try{
    ...
    else if(act==="delete-website-project")await deleteWebsiteProject(id);
    else if(act==="delete-project-asset")await deleteWebsiteProjectAsset(a.dataset.path,a.dataset.projectId);
    else if(act==="copy-project-intake")await copyProjectIntake(a.dataset.clientId,id);
    else if(act==="manage-project-showcase")await openProjectShowcaseEditor(a.dataset.clientId,id);
    else if(act==="save-project-showcase")await saveProjectShowcase();
    else if(act==="remove-showcase-logo")await removeShowcaseLogo(id);
    else if(act==="remove-showcase-image")await removeShowcaseImage(id,Number(a.dataset.index));
...
document.addEventListener("input",e=>{if(e.target.id==="clientSearch")renderClientRows();if(e.target.id==="promptSearch")renderPromptRows();if(e.target.matches?.("textarea.auto-grow"))autoGrow(e.target);if(["invoiceClient","invoiceMode","invoiceStart","invoiceEnd","invoiceManualHours","invoiceRate"].includes(e.target.id))updateInvoicePreview();if(["timeEditStart","timeEditEnd"].includes(e.target.id))updateTimeEditPreview()});
