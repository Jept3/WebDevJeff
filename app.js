const BUILD="v2.18-system-audit-asset-library";
...
async function loadEmployerProjectAssets(project,editable){const c=currentClient(),box=$("websiteProjectAssetsList");if(!c||!project||!box)return;const files=await listWebsiteProjectAssets(c.id,project.id);if(!box.isConnected)return;box.innerHTML=assetLibraryHtml(files,editable)}
...
async function renderAdminWebsiteProject(clientId,projectId){
  ...
  const project=await getWebsiteProject(projectId);if(!project||seq!==state.navSeq)return;const assets=await listWebsiteProjectAssets(clientId,projectId);if(seq!==state.navSeq)return;const gallery=showcaseGalleryPaths(project),urls=await Promise.all([project.showcase_logo_path,...gallery].filter(Boolean).map(p=>signedFileUrl(p)));if(seq!==state.navSeq)return;let cursor=0;const logoUrl=project.showcase_logo_path?urls[cursor++]:"",galleryUrls=gallery.map(()=>urls[cursor++]).filter(Boolean);
  ...
  <article class="card glass admin-branding-panel"><span class="section-label">BRANDING & GALLERY</span><h3>Logo and screenshots</h3><div class="admin-logo-stage">${logoUrl?`<div class="logo-proof"><div class="logo-proof-head"><div><span class="section-label">LOGO CONTRAST CHECK</span><h4>Current Website Logo</h4></div><button class="mini-btn" data-action="open-showcase-image" data-url="${esc(logoUrl)}" data-mode="logo">Enlarge</button></div><div class="logo-proof-grid"><button class="logo-proof-panel light" data-action="open-showcase-image" data-url="${esc(logoUrl)}" data-mode="logo"><span>Light background</span><img src="${esc(logoUrl)}" alt="Website logo on light background"></button><button class="logo-proof-panel dark" data-action="open-showcase-image" data-url="${esc(logoUrl)}" data-mode="logo"><span>Dark background</span><img src="${esc(logoUrl)}" alt="Website logo on dark background"></button></div></div>`:'<div class="empty logo-empty"><strong>No logo uploaded</strong><span>Upload a final logo to preview it automatically on light and dark backgrounds.</span></div>'}</div><label class="website-dropzone"><strong>＋ Upload / Replace Logo</strong><span>PNG, JPG, WEBP, SVG or HEIC · up to 25 MB</span><input id="adminShowcaseLogoInput" type="file" accept="image/*,.svg,.heic,.heif" hidden></label><div id="adminLogoUploadStatus" class="upload-status muted">Choose a logo to upload.</div><label class="website-dropzone"><strong>＋ Add Project Screenshots</strong><span>Select multiple images for the Employer carousel</span><input id="adminShowcaseGalleryInput" type="file" multiple accept="image/*,.heic,.heif" hidden></label><div id="adminGalleryUploadStatus" class="upload-status muted">Choose one or more screenshots to upload.</div><div class="admin-gallery-grid">${galleryUrls.length?galleryUrls.map((url,i)=>`<div class="showcase-admin-shot"><button data-action="open-showcase-image" data-url="${esc(url)}" data-mode="screenshot"><img src="${esc(url)}" alt="Screenshot ${i+1}"></button><button class="mini-btn danger" data-action="remove-showcase-image" data-id="${project.id}" data-index="${i}" data-return="admin-workspace">Remove</button></div>`).join(""):'<div class="empty"><strong>No screenshots yet</strong>The Employer carousel will appear after you upload screenshots and publish the preview.</div>'}</div></article></div>
...
async function buildShowcaseView(project){
  ...
  return `<section class="project-showcase glass"><div class="showcase-top"><div><span class="section-label">ONGOING WEBSITE PREVIEW</span><h2>${esc(project.website_name||"Website Project")}</h2><p class="muted">${esc(project.showcase_notes||"Preview the latest design direction and website progress below.")}</p>${site!=="#"?`<a class="btn primary showcase-site-link" href="${esc(site)}" target="_blank" rel="noopener">Open Website ↗</a>`:""}</div>${logoUrl?`<div class="showcase-logo-proof"><div class="logo-proof-head"><div><span class="section-label">APPROVED BRAND MARK</span><h3>Website Logo</h3></div><button class="mini-btn" data-action="open-showcase-image" data-url="${esc(logoUrl)}" data-mode="logo">Enlarge</button></div><div class="logo-proof-grid"><button class="logo-proof-panel light" data-action="open-showcase-image" data-url="${esc(logoUrl)}" data-mode="logo"><span>Light</span><img src="${esc(logoUrl)}" alt="${esc(project.website_name||"Website")} logo on light background"></button><button class="logo-proof-panel dark" data-action="open-showcase-image" data-url="${esc(logoUrl)}" data-mode="logo"><span>Dark</span><img src="${esc(logoUrl)}" alt="${esc(project.website_name||"Website")} logo on dark background"></button></div></div>`:""}</div>${galleryUrls.length?`<div class="showcase-gallery-head"><div><span class="section-label">LATEST SCREENSHOTS</span><h3>Website Gallery</h3></div><span class="muted">Tap any image to enlarge</span></div><div class="showcase-carousel" data-showcase-carousel><div class="showcase-track">${galleryUrls.map((url,i)=>`<button class="showcase-slide" data-action="open-showcase-image" data-url="${esc(url)}" data-mode="screenshot" aria-label="Open screenshot ${i+1}"><img src="${esc(url)}" alt="Website screenshot ${i+1}" loading="lazy"><span>${String(i+1).padStart(2,"0")}</span></button>`).join("")}</div><div class="showcase-dots">${galleryUrls.map((_,i)=>`<i class="${i===0?"active":""}"></i>`).join("")}</div></div>`:""}</section>`;
}
...
document.addEventListener("change",e=>{if(e.target.id==="clientSort")renderClientRows();if(e.target.id==="taskFilter")renderTaskInboxRows();if(["invoiceClient","invoiceMode","invoiceStart","invoiceEnd","invoiceManualHours","invoiceRate"].includes(e.target.id))updateInvoicePreview()});
