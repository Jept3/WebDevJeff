const BUILD="v2.17.2-storage-rls-hotfix";
...
async function uploadFiles(files){const c=currentClient();if(!c)return toast("No employer project linked");const maxBytes=25*1024*1024;try{for(const f of files){if(f.size>maxBytes)throw new Error(`${f.name} exceeds the 25 MB limit`);const safe=(f.name||"upload").replace(/[^\w.\- ]+/g,"_");const name=`${Date.now()}-${crypto.randomUUID().slice(0,8)}-${safe}`;const {error}=await state.sb.storage.from("client-files").upload(`${c.id}/${name}`,f,{upsert:false,contentType:f.type||undefined});if(error)throw error}toast("Files uploaded");renderEmployer(state.employerView)}catch(error){console.error("Employer file upload failed",error);toast(error.message||"Upload failed")}}
