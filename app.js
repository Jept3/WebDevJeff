...
function websiteProjectEmployerCard(project,editable){const progress=projectCompletion(project);return `<article class="website-project-card glass visual-project-card"><div class="website-project-card-head"><div class="project-card-title"><span class="section-label">WEBSITE PROJECT</span><h3>${esc(project.website_name||"Untitled Website")}</h3><p class="muted">Updated ${fmtDate(project.updated_at)}</p></div>${progressRing(progress,"Intake",project.showcase_published?"Preview available":"Submitted")}</div><div class="project-progress-line"><span><b>${progress}%</b> intake prepared</span><i><em style="width:${progress}%"></em></i></div><div class="project-card-meta">${websiteProjectStatusPill(project.status)}${project.showcase_published?'<span class="status complete">● Preview available</span>':'<span class="status">Admin-managed status</span>'}</div><div class="website-project-card-actions"><button class="btn primary" data-action="open-website-project" data-id="${project.id}">${editable?"Open & Update Intake":"View Project"}</button></div></article>`}
...
async function renderEmployerWebsiteProjectEditor(project,editable){
  const c=currentClient(),seq=state.navSeq;const showcase=await buildShowcaseView(project);if(seq!==state.navSeq)return;const intake=project.website_intake&&typeof project.website_intake==="object"?project.website_intake:{};
  $("view").innerHTML=`${showcase}<div class="project-editor-top"><button class="btn ghost" data-action="back-to-website-projects">← All Website Projects</button></div>
