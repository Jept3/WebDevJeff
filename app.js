const BUILD="v2.17.3-storage-rls-hotfix";
...
function renderAdmin(view){
  ...
  const map={dashboard:()=>renderDashboard(),clients:()=>renderClients(),websites:()=>renderAdminWebsiteProjects(),prompts:()=>renderPromptLibrary(),tasks:()=>renderTaskInbox(),time:()=>renderTimePage(),invoices:()=>renderAdminInvoices(),billing:()=>renderBilling(),trash:()=>renderTrash()};
...
async function renderEmployerWebsiteProjectViewer(project){
  ...
  const showcase=await buildShowcaseView(project);if(seq!==state.navSeq)return;
...
async function renderEmployerWebsiteProjectEditor(project,editable){
  ...
  <article class="card glass website-assets-panel" style="margin-top:13px"><div class="section-head"><div><span class="section-label">PROJECT ASSETS</span><h3>Logos, photos & website materials</h3><p class="muted">Upload logos, photos, screenshots, PDFs and other materials specifically for this website project.</p></div></div><div id="websiteProjectAssetsList" class="file-grid website-assets"><div class="empty"><strong>Loading project assets…</strong></div></div>${editable?`<label id="websiteAssetDrop" class="website-dropzone employer-upload-drop"><strong>＋ Upload Project Assets</strong><span>Choose files or drag them here · up to 25 MB each</span><input id="websiteAssetInput" type="file" multiple hidden></label><div id="websiteAssetUploadStatus" class="upload-status muted">No upload in progress.</div>`:""}</article>
...
async function loadEmployerProjectAssets(project,editable){const c=currentClient(),box=$("websiteProjectAssetsList");if(!c||!project||!box)return;const files=await listWebsiteProjectAssets(c.id,project.id);if(!box.isConnected)return;box.innerHTML=websiteProjectAssetListHtml(files,c,project,editable)}
async function uploadWebsiteProjectAssets(files,projectId){const c=currentClient(),status=$("websiteAssetUploadStatus"),input=$("websiteAssetInput");if(!c||!files?.length||!projectId)return;const maxBytes=25*1024*1024;const allowedCount=files.length;if(status){status.textContent=`Uploading ${allowedCount} file${allowedCount===1?"":"s"}…`;status.classList.remove("upload-error","upload-success")}try{for(const f of files){if(f.size>maxBytes)throw new Error(`${f.name} exceeds the 25 MB limit`);const safe=(f.name||"upload").replace(/[^\w.\- ]+/g,"_");const name=`${Date.now()}-${crypto.randomUUID().slice(0,8)}-${safe}`;const path=`${c.id}/website-projects/${projectId}/${name}`;const {error}=await state.sb.storage.from("client-files").upload(path,f,{upsert:false,contentType:f.type||undefined});if(error)throw error}if(status){status.textContent=`Uploaded ${allowedCount} file${allowedCount===1?"":"s"} successfully.`;status.classList.add("upload-success")}toast("Project assets uploaded");const project=await getWebsiteProject(projectId);if(project)await loadEmployerProjectAssets(project,true)}catch(error){console.error("Employer project asset upload failed",error);const raw=error?.message||"Upload failed";const friendly=/row-level security|violates.*policy/i.test(raw)?"Upload blocked by Storage permissions. Run the v2.17.3 Employer Project Asset RLS patch in Supabase, then try again.":raw;if(status){status.textContent=friendly;status.classList.add("upload-error")}toast(friendly)}finally{if(input)input.value=""}}
async function deleteWebsiteProjectAsset(path,projectId){if(!confirm("Delete this project asset?"))return;const {error}=await state.sb.storage.from("client-files").remove([path]);if(error)return toast(error.message);toast("Asset deleted");const project=await getWebsiteProject(projectId);if(project)await loadEmployerProjectAssets(project,true)}
...
async function renderAdminWebsiteProjects(){
  ...
  if(seq!==state.navSeq)return;
  const projects=data||[],draft=projects.filter(p=>p.status==="draft").length,progress=projects.filter(p=>p.status==="in_progress").length,ready=projects.filter(p=>p.status==="ready").length,complete=projects.filter(p=>p.status==="complete").length;
  const rows=projects.map(p=>{const c=state.clients.find(x=>x.id===p.client_id);return `<article class="admin-website-list-card glass"><div class="admin-website-list-main"><div><span class="section-label">${esc(c?.name||"EMPLOYER")}</span><h3>${esc(p.website_name||"Untitled Website")}</h3><p class="muted">${esc(c?.company||c?.name||"Employer")} · Updated ${fmtDate(p.updated_at)}</p></div><div class="admin-website-card-meta">${websiteProjectStatusPill(p.status)}${p.showcase_published?'<span class="status complete">Preview Live</span>':''}<span class="status">${projectCompletion(p)}% Intake</span></div></div><div class="project-progress-line"><i style="width:${projectCompletion(p)}%"></i></div><div class="admin-website-card-actions"><button class="btn primary" data-action="open-admin-website-project" data-id="${p.id}" data-client-id="${p.client_id}">Open Admin Workspace</button><button class="btn ghost" data-action="copy-project-intake" data-id="${p.id}" data-client-id="${p.client_id}">Copy Intake</button>${c?`<button class="mini-btn" data-action="client-detail" data-id="${c.id}">Employer</button>`:""}</div></article>`}).join("");
  $("view").innerHTML=`<section class="page-shell admin-websites-page"><div class="page-head"><div><span class="section-label">PRODUCTION CONTROL</span><h1>Website Projects</h1><p class="muted">Manage every Employer website from intake through launch in one place.</p></div></div><div class="visual-stats-grid admin-website-stats">${metricVisual("◌","Drafting",draft,"Waiting / planning","lime")}${metricVisual("↗","In Progress",progress,"Currently being built","blue")}${metricVisual("✓","Ready to Build",ready,"Intake ready","gold")}${metricVisual("◆","Completed",complete,"Delivered projects","violet")}</div><div class="admin-website-list">${rows||'<article class="card glass empty-project-state"><strong>No website projects yet</strong><p class="muted">Employer-created projects will appear here automatically.</p></article>'}</div></section>`;
}
...
async function uploadFiles(files){const c=currentClient();if(!c)return toast("No employer project linked");const maxBytes=25*1024*1024;try{for(const f of files){if(f.size>maxBytes)throw new Error(`${f.name} exceeds the 25 MB limit`);const safe=(f.name||"upload").replace(/[^\w.\- ]+/g,"_");const name=`${Date.now()}-${crypto.randomUUID().slice(0,8)}-${safe}`;const {error}=await state.sb.storage.from("client-files").upload(`${c.id}/${name}`,f,{upsert:false,contentType:f.type||undefined});if(error)throw error}toast("Files uploaded");renderEmployer(state.employerView)}catch(error){console.error("Employer file upload failed",error);const raw=error?.message||"Upload failed";const friendly=/row-level security|violates.*policy/i.test(raw)?"Upload blocked by Storage permissions. Run the v2.17.3 Unified Employer Storage RLS patch in Supabase, then try again.":raw;toast(friendly)}}
