-- Jeffdesign101 CLEAN MIGRATION v2
...
create index if not exists client_tasks_client_created_idx on public.client_tasks(client_id, created_at desc);
create index if not exists client_tasks_open_idx on public.client_tasks(client_id, done, created_at desc);
create index if not exists time_entries_client_clock_idx on public.time_entries(client_id, clock_in desc);
create index if not exists time_entries_open_idx on public.time_entries(client_id, clock_in desc) where clock_out is null;
create index if not exists invoices_client_date_idx on public.invoices(client_id, invoice_date desc);
...
as $$
begin
  insert into public.activity_log(actor_id,action,entity_type,entity_id,old_data,new_data)
...
-- Avoid duplicate audit triggers on reruns.
drop trigger if exists audit_clients on public.clients;
drop trigger if exists audit_tasks on public.client_tasks;
drop trigger if exists audit_time on public.time_entries;
drop trigger if exists audit_invoices on public.invoices;
create trigger audit_clients after insert or update or delete on public.clients for each row execute procedure public.audit_row_change();
create trigger audit_tasks after insert or update or delete on public.client_tasks for each row execute procedure public.audit_row_change();
